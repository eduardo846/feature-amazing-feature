...
/// Reject missing/weak JWT secrets. Call once at startup so a misconfigured
/// deployment fails immediately instead of silently signing tokens with a
/// well-known default secret.
fn validate_jwt_secret_strength(jwt_secret: &str) -> Result<(), String> {
...
async fn main() -> std::io::Result<()> {
    ...
    if let Err(reason) = validate_jwt_secret_strength(&std::env::var("JWT_SECRET").unwrap()) {
        let err_msg = format!("Fatal: JWT_SECRET is invalid: {}. Service cannot start.", reason);
        tracing::error!("{}", err_msg);
        return Err(std::io::Error::other(err_msg));
    }
...
mod tests {
    ...
    async fn test_verify_auth_signature_endpoint() {
        use actix_web::test;
        std::env::set_var("JWT_SECRET", "default_secret_for_development_only");
        let store: RefreshStore = Arc::new(Mutex::new(HashMap::new()));
        let app = test::init_service(
            App::new()
