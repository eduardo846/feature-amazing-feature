...
/**
 * Resolve the first secret in `names` that is configured, in order.
 * Useful when a value has historically been sourced from more than one
 * secret name (e.g. a JWT secret that may be shared with NEXTAUTH_SECRET).
 * Throws only if none of them resolve — never falls back to a hardcoded value.
 */
export async function getSecretWithFallback(names: SecretName[]): Promise<string> {
  ...
  throw lastError ?? new Error('KMS: no secret names provided');
}
