 ...
 * Must run inside the same Prisma transaction as the mutations that follow —
 * a `SELECT ... FOR UPDATE` issued outside a transaction releases the lock
...
/**
 * Acquire locks on multiple rows in strict order, inside `tx`.
 * This prevents circular wait deadlocks and keeps each lock held until the
...
export async function acquireLocksInOrder(
  ...
  for (const lock of sorted) {
    ...
    try {
      ...
      switch (lock.type) {
        case "creator":
          result = await lockCreator(tx, lock.id, mode);
          break;
        case "client":
          result = await lockClient(tx, lock.id, mode);
          break;
        case "escrow":
          result = await lockEscrow(tx, lock.id, mode);
          break;
        case "balance":
          result = await lockBalance(tx, lock.id, mode);
          break;
      }
