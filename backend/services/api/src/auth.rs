...
//! There is no insecure fallback: `main()` validates at startup (see
//! `validate_jwt_secret_strength`) that it is set and not a known weak/dev
//! value, so any deployment missing it fails fast instead of accepting
//! tokens forged with a publicly-known secret.
...
fn jwt_secret() -> String {
    std::env::var("JWT_SECRET")
        .expect("JWT_SECRET must be set — validate_jwt_secret_strength() should have been called at startup")
}

/// Reject missing/weak JWT secrets. Call once at startup (see `main.rs`) so a
/// misconfigured deployment fails immediately instead of silently accepting
/// tokens forged with a well-known default secret.
pub fn validate_jwt_secret_strength(jwt_secret: &str) -> Result<(), String> {
    ...
    {
        return Err("JWT_SECRET must not use a default or placeholder value".to_string());
    }
...
pub mod tests {
    ...
    fn valid_token_decodes_claims() {
        std::env::set_var("JWT_SECRET", "stellar-dev-secret");
